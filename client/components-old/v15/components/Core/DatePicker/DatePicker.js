...
renderMonthText: mutuallyExclusiveProps(
	PropTypes.func, 'renderMonthText', 'renderMonthElement'
...
renderMonthElement: mutuallyExclusiveProps(
	PropTypes.func, 'renderMonthText', 'renderMonthElement'
	), PropTypes.func, // ({ month, onMonthSelect, onYearSelect, isVisible }) => PropTypes.node,
...
const DatePickerContainer = styled.div`
	...
	{/* This is here because Parcel isn't importing the exertnal CSS file during build :(  */}
	...
	{/* ============ MOS custom styles below ============ */}

	div.DayPicker {
		background: transparent;
		box-shadow: unset;
		color: ${Style.Color({ token: 'white' })};
		div.DayPicker_weekHeaders {
			li.DayPicker_weekHeader_li {
				color: ${Style.Color({ token: 'blue-2' })};
			}
		}
		div.DayPicker_focusRegion {
			div.DayPickerNavigation {
						...
						table.CalendarMonth_table {
							tbody {
								...
								td.CalendarDay__default {
									...
									background: transparent;
									border-color: transparent;
									color: ${Style.Color({ token: 'subtle-yellow' })};
									transition: all .25s;
									&:hover {
										color: ${Style.Color({ token: 'bold-blue' })};
										background: ${Style.Color({ token: 'subtle-yellow' })};
									}
								}
								td.CalendarDay__selected {
									color: ${Style.Color({ token: 'bold-blue' })};
									background: ${Style.Color({ token: 'subtle-blue' })};
									&:hover {
										color: ${Style.Color({ token: 'bold-blue' })};
										background: ${Style.Color({ token: 'subtle-blue' })};
									}
								...
								td.CalendarDay__blocked_out_of_range {
									color: ${Style.Color({ token: 'blue-7' })};
									&:hover {
										color: ${Style.Color({ token: 'blue-7' })};
										background: transparent;
									}
								}
								td.CalendarDay__today:not(.CalendarDay__selected) {
									background: ${Style.Color({ token: 'blue-12' })};
									&:hover {
										color: ${Style.Color({ token: 'bold-blue' })};
										background: ${Style.Color({ token: 'subtle-yellow' })};
									}
	...
	${(props) => props.selectionType === 'single' && `
		div.SingleDatePicker div.SingleDatePickerInput {
 ...
 */
const IconBackward = ({ device, svgsAll }) => (
	<Icon
		color="subtle-yellow"
		size="s"
...
const IconForward = ({ device, svgsAll }) => (
	<Icon
		color="subtle-yellow"
		size="s"
...
const DatePicker = ({
	...
	const HandleDatePicked = (momentObject) => {
		callBack();
		handler(momentObject);
	};
	return (
		<DatePickerContainer
		...
		>
			{
				...
				(
					<SingleDatePicker
						numberOfMonths={scope.quantityMonthsInView}
						isOutsideRange={(day) => day.isBefore(moment(scope.selectableDaysStartDate), 'day') || day.isAfter(moment(scope.selectableDaysStartDate).add(scope.selectableDaysLength, 'days'), 'day')}
						date={dates.startDate}
						onDateChange={HandleDatePicked}
						focused={focused}
						onFocusChange={({ isFocused }) => null}
						id="SingleDatePickerDateInput"
						noBorder
						navPrev={(
							<IconBackward
								device={device}
