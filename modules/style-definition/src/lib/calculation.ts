...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
				...
				styleObjects.forEach((styleObject) => {
					...
					if (propertiesArray[2] === 'Light') {
						...
						allColors[propertiesArray[2]][propertiesArray[3]][
							parseInt(propertiesArray[4])] = thisColorHSLA;
					}
		...
		if (styleSetFunction) {
			...
			styleSetFunction()
				...
				.then((result:AllColors | AllTypeStyles | AllShadows) => {
					...
					const styleLocation =
						foundation.extraction.definitions.path +
...
export const cloneFoundation = ():Promise<{ error: boolean }> =>
	// return a new, main promise
	new Promise((resolve, reject) => {
		...
		const foundationString =
			`export const foundation = ${JSON.stringify(foundationClone)};`;
		const styleLocation =
			foundation.extraction.definitions.path +
		...
		fs.writeFile(styleLocation, foundationString, (error) => {
			if (error) {
				reject(new Error(
					`cloneFoundation: could not write file - \
...
export const buildStyleServiceStore = ():Promise<{ error: boolean }> =>
	// return a new, main promise
	new Promise((resolve, reject) => {
