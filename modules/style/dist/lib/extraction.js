    ...
    axios({
        ...
        if (result
            ...
            const figmaStylePages = result.data.document.children
                .filter((page) => Object.values(foundation.figma.pageTitles).includes(page.name));
...
new Promise((resolve, reject) => {
    fs.readFile(`${foundation.storage.path}${foundation.storage.names.figma}`, 'utf8', (error, jsonString) => {
        if (error) {
            reject(error);
        }
