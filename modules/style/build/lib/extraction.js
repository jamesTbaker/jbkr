...
new Promise((resolve, reject) => {
    ...
    fetchFigmaStylePages()
        ...
        const filePath = `${styleDefinition.storage.path}${styleDefinition.storage.names.figma}`;
        try {
            // write data to file
            ...
            // reject this promise with the error
            reject(new Error('storeFigmaStyleObjects: could not write file'));
        }
        // } else {
        // 	// reject this promise with the error
        // 	reject(new Error('storeFigmaStyleObjects: figmaStylePages not found'));
        // }
    })
        // if the promise is rejected with an error
        .catch((error) => {
